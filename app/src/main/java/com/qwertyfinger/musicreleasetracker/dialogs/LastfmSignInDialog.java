...
import com.qwertyfinger.musicreleasetracker.events.lastfm.ThresholdInvalidEvent;
import com.qwertyfinger.musicreleasetracker.events.lastfm.UsernameInvalidEvent;
        ...
        App.getInstance().getJobManager().addJob(new Job(new Params(Constants.JOB_PRIORITY_CRITICAL)) {
            ...
            public void onRun() throws Throwable {
                ...
                if ((!(user == null)) && thresholdInt >= 0) {
                    ...
                    if (user == null) {
                        EventBus.getDefault().post(new UsernameInvalidEvent());
                        field.setAccessible(true);
                        field.set(dialog, true);
                    } else {
                        EventBus.getDefault().post(new ThresholdInvalidEvent());
                        field.setAccessible(true);
                        field.set(dialog, true);
                    }
                }
