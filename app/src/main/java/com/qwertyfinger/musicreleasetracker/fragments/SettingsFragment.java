...
import com.qwertyfinger.musicreleasetracker.receivers.DeezerSyncReceiver;
import com.qwertyfinger.musicreleasetracker.receivers.LastfmSyncReceiver;
        ...
        if (key.equals(SYNC_FREQUENCY)) {
            ...
            if (frequency != 0) {
                if (settings.getBoolean(SettingsFragment.DEEZER, false)) {
                    Intent receiverIntent = new Intent(getActivity(), DeezerSyncReceiver.class);
                    PendingIntent alarmIntent = PendingIntent.getBroadcast(getActivity().getApplicationContext(), 0,
                            receiverIntent,
                            0);
                ...
                if (settings.getBoolean(SettingsFragment.RELEASE_REFRESH_ALARM_SET, false)) {
                    Intent receiverIntent = new Intent(getActivity(), NewDayReceiver.class);
                    PendingIntent alarmIntent = PendingIntent.getBroadcast(getActivity().getApplicationContext(), 0,
                            receiverIntent, 0);
        ...
        if (key.equals(DEEZER)) {
            if (sharedPreferences.getBoolean(DEEZER, false)) {
                RequestListener listener = new JsonRequestListener() {
                    ...
                    public void onResult(Object result, Object requestId) {
                        ...
                        if (frequency != 0) {
                            ...
                            Intent receiverIntent = new Intent(getActivity(), DeezerSyncReceiver.class);
                            PendingIntent alarmIntent = PendingIntent.getBroadcast(App.getInstance().getApplicationContext(), 0,
                                    receiverIntent, 0);

                            alarmMgr.setInexactRepeating(AlarmManager.ELAPSED_REALTIME, SystemClock.elapsedRealtime() - 100000,
                                    frequency * AlarmManager.INTERVAL_DAY + Utils.generateRandom() * 1000 - Utils
                                            .generateRandom() * 1000, alarmIntent);
                            editor.putBoolean(SettingsFragment.DEEZER_SYNC_ALARM_SET, true);
                        }
                        else
                ...
                if (settings.getBoolean(SettingsFragment.DEEZER_SYNC_ALARM_SET, false)) {
                    AlarmManager alarmMgr = (AlarmManager) getActivity().getSystemService(Context.ALARM_SERVICE);
                    Intent receiverIntent = new Intent(getActivity(), DeezerSyncReceiver.class);
                    PendingIntent alarmIntent = PendingIntent.getBroadcast(App.getInstance().getApplicationContext(), 0, receiverIntent, 0);
                    alarmMgr.cancel(alarmIntent);
