...
public class SyncLastfmJob extends Job {
    ...
    private int i;
    private List<com.qwertyfinger.musicreleasetracker.entities.Artist> finalArtists;
        ...
        if (!Utils.isSyncInProgress(context) && Utils.isExternalStorageWritable() && Utils.isConnected(context)) {
            ...
            if (lastfmArtists.size() > 0) {
                ...
                if (finalArtists.size() > 0) {
                    ...
                    List<com.qwertyfinger.musicreleasetracker.entities.Artist> partList = new ArrayList<>();
                    for (int k = 0; k < 50; k++) {
